...
class DiziPal : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val doc = app.get(request.data.addPage(page), interceptor = interceptor).document
        val items = when {
    ...
    /** /yeni-eklenen-bolumler’den gelen <a href="/bolum/..."> öğesini
     *  dizi kartına çevirir: /series/<slug>
     */
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        // 1) Eski JSON autocomplete (varsa)
        runCatching {
            val res = app.post(
                "$mainUrl/api/search-autocomplete",
                headers     = mapOf("Accept" to "application/json, text/javascript, */*; q=0.01",
        ...
        }.onFailure { /* fallback'e geç */ }

        // 2) HTML arama sayfası (/arama-yap)
        val doc = app.get("$mainUrl/arama-yap?keyword=${query.encodeURL()}",
            interceptor = interceptor, referer = "$mainUrl/").document
        val results = mutableListOf<SearchResponse>()
        doc.select("a[href^=\"/series/\"]").forEach { toSeries ->
            toSeries.toSeriesCard()?.let(results::add)
        }
        doc.select("a[href^=\"/movies/\"]").forEach { toMovie ->
            toMovie.toMovieCard()?.let(results::add)
        }
        return results
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val titleMeta   = doc.selectFirst("h1, .g-title div, .title h1, meta[property='og:title']")?.let {
    ...
    override suspend fun loadLinks(
        ...
        page.select("iframe[src]").forEach { iframe ->
            ...
            Regex("""file\s*:\s*"([^"]+\.m3u8[^"]*)""").find(body)?.groupValues?.getOrNull(1)?.let { m3u ->
                callback.invoke(
                    ExtractorLink(
                        source  = name,
                        name    = name,
        ...
        // 2) Sayfa içinde m3u8
        Regex("""file\s*:\s*"([^"]+\.m3u8[^"]*)""").find(page.html())?.groupValues?.getOrNull(1)?.let { m3u ->
            callback.invoke(
                ExtractorLink(
                    source  = name,
                    name    = name,
